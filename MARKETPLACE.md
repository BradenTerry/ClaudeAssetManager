...
example, drop a whole skill folder onto another project to reuse it. Copies are type-constrained:
a skill only lands in **skills**, an agent in **agents**, a command in **commands**. Drop onto a
matching type folder, or onto a project / added-directory folder to route each item to the right
place automatically. If something with the same name already exists there, you are asked before it
is overwritten. Dragging copies (it never deletes the original).
